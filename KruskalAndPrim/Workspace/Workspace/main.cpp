...
} closedge[VertexNumber]; //Prim算法中的辅助信息
...
void AdjMatrix(unsigned int adjMat[][VertexNumber])  //邻接矩阵表示法
{
    for (int i = 0; i < VertexNumber; i++)   //初始化邻接矩阵
...
void MiniSpanTree_Prim(unsigned int adjMat[][VertexNumber], VertexData s)
{
    for (int i = 0; i < VertexNumber;i++)
    {
    ...
    for (int e = 1; e <= VertexNumber -1; e++)  //n-1条边时退出
    {
        int k = Minmum(closedge);  //选择最小代价边
        cout << vertexID[closedge[k].data] << "--" << vertexID[k] << endl;//加入到最小生成树
        closedge[k].lowestcost = 0; //代价置为0
        for (int i = 0; i < VertexNumber;i++)  //更新v中顶点最小代价边信息
        {
...
void ReadArc(unsigned int  adjMat[][VertexNumber],vector<Arc> &vertexArc) //保存图的边代价信息
{
    Arc * temp = NULL;
...
void MiniSpanTree_Kruskal(unsigned int adjMat[][VertexNumber])
{
    vector<Arc> vertexArc;
    ReadArc(adjMat, vertexArc);//读取边信息
    sort(vertexArc.begin(), vertexArc.end(), compare);//边按从小到大排序
    vector<vector<VertexData> > Tree(VertexNumber); //6棵独立树
    for (unsigned int i = 0; i < VertexNumber; i++)
    {
    ...
    for (unsigned int i = 0; i < vertexArc.size(); i++)//依次从小到大取最小代价边
    {
        ...
        {
            cout << vertexID[u] << "---" << vertexID[v] << endl;//把此边加入到最小生成树中
        }
    }
}
...
int main()
{
    unsigned int  adjMat[VertexNumber][VertexNumber] = { 0 };
